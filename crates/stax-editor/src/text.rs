...
static BUILTIN_DOCS: &[(&str, &str, &str)] = &[
    ...
    (
        "sinosc",
        "freq phase → Signal",
        "Sine oscillator (Hz, initial phase 0..1)",
    ),
    (
        "saw",
        "freq phase → Signal",
        "Bandlimited sawtooth (Hz, phase 0..1)",
    ),
    (
        "lfsaw",
        "freq phase → Signal",
        "Non-bandlimited sawtooth LFO",
    ),
    ...
    (
        "dust",
        "density → Signal",
        "Random impulses at avg density Hz",
    ),
    ("dust2", "density → Signal", "Bipolar random impulses"),
    (
        "sah",
        "sig trig → Signal",
        "Sample-and-hold: freeze sig on trig",
    ),
    // filters
    ...
    (
        "rlpf",
        "sig cutoff rq → Signal",
        "Resonant lowpass (rq = 1/Q)",
    ),
    (
        "rhpf",
        "sig cutoff rq → Signal",
        "Resonant highpass (rq = 1/Q)",
    ),
    (
        "svflp",
        "sig freq q → Signal",
    ...
    (
        "lag",
        "sig t → Signal",
        "1-pole lowpass smoothing (time const t)",
    ),
    ...
    (
        "firlp",
        "sig cutoff n → Signal",
        "Windowed-sinc FIR lowpass (n taps)",
    ),
    (
        "firhp",
        "sig cutoff n → Signal",
        "Windowed-sinc FIR highpass",
    ),
    (
        "firbp",
        "sig lo hi n → Signal",
        "Windowed-sinc FIR bandpass",
    ),
    (
        "hilbert",
        "sig n → Signal",
        "Hilbert FIR transformer (n taps)",
    ),
    // envelopes
    ...
    (
        "hanenv",
        "t → Signal",
        "Hann-window envelope over t seconds",
    ),
    (
        "decay",
        "t → Signal",
        "Exponential decay from 1 to ~0 in t sec",
    ),
    ("decay2", "t60 → Signal", "Decay to -60 dB in t60 seconds"),
    (
        "line",
        "start end dur → Signal",
        "Linear ramp from start to end over dur sec",
    ),
    (
        "xline",
        "start end dur → Signal",
        "Exponential ramp start→end over dur sec",
    ),
    // dynamics / delay
    (
        "combn",
        "sig delay maxdel decay → Sig",
        "Comb filter (N samples delay)",
    ),
    ("delayn", "sig n max → Signal", "Simple N-sample delay line"),
    (
        "verb",
        "sig n decay room → Signal",
        "FDN reverb (n=2..16 delay lines)",
    ),
    (
        "compressor",
        "sig thr ratio atk rel → Sig",
    ...
    (
        "pan3",
        "sig az el → [L C R]",
        "3-channel pan (azimuth, elevation)",
    ),
    // synthesis
    (
        "pluck",
    ...
    (
        "grain",
        "sig rate dur → Signal",
        "Granular synthesis from signal",
    ),
    // analysis
    ...
    (
        "bench",
        "n word → Real",
        "Benchmark word n times, return ms",
    ),
    // stack
    ...
    (
        "while",
        "cond body →",
        "Loop while cond leaves truthy value",
    ),
    ...
    (
        "by",
        "start step → Stream",
        "Arithmetic sequence from start, step δ",
    ),
    (
        "nby",
        "start step n → Stream",
    ...
    (
        "N",
        "stream n → Stream",
        "Take first n elements (postfix alias)",
    ),
    ...
    (
        "keepWhile",
        "fn stream → Stream",
        "Keep prefix while fn returns truthy",
    ),
    (
        "skipWhile",
        "fn stream → Stream",
        "Drop prefix while fn returns truthy",
    ),
    ...
    (
        "grade",
        "stream → Stream",
        "Indices that would sort the stream",
    ),
    ("flatten", "stream → Stream", "Flatten one level of nesting"),
    (
        "mirror",
        "stream → Stream",
        "Palindrome: append reversed copy",
    ),
    ...
    (
        "2X",
        "stream → Stream",
        "Double: each element appears twice",
    ),
    (
        "grow",
        "n stream → Stream",
        "Grow by repeating last element to len n",
    ),
    ...
    (
        "local",
        "key form → val",
        "Look up key in form, no parent chain",
    ),
    (
        "dot",
        "key form → val",
        "Look up key following parent chain",
    ),
    ...
    (
        "linexp",
        "x lo hi out_lo out_hi → y",
        "Linear input → exponential output",
    ),
    (
        "explin",
        "x lo hi out_lo out_hi → y",
        "Exponential input → linear output",
    ),
...
impl StaxApp {
    ...
    pub(crate) fn draw_files_panel(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if let Some(err) = &self.parse_error.clone() {
                ...
                let suggestion = extract_unknown_word(err).and_then(best_suggestion);
    ...
    pub(crate) fn draw_text_editor(&mut self, ui: &mut egui::Ui) {
        ...
        let gutter_rect =
            Rect::from_min_size(full_code_rect.min, vec2(GUTTER_W, full_code_rect.height()));
        ...
        let mut layouter =
            |ui: &egui::Ui, s: &str, _wrap_width: f32| -> std::sync::Arc<egui::Galley> {
        ...
        let ctrl_space = ui.input_mut(|i| i.consume_key(egui::Modifiers::CTRL, egui::Key::Space));
        if ctrl_space {
        ...
        if self.show_completion && !self.completion_candidates.is_empty() {
            let popup_x = editor_rect.min.x + (self.cursor_col.saturating_sub(1)) as f32 * CHAR_W;
            let popup_y =
                editor_rect.min.y + (self.cursor_line as f32) * ROW_H - scroll_y + ROW_H * 0.5;
            let popup_pos = egui::pos2(
                popup_x,
                popup_y.clamp(editor_rect.min.y, editor_rect.max.y - 120.0),
            );

            // Clone to avoid borrow conflict with self.recompile() inside closure
            let candidates: Vec<String> =
                self.completion_candidates.iter().take(8).cloned().collect();
        ...
        for line in (visible_start + 1)..=(visible_end + 1) {
            ...
            if let Some((err_line, err_col)) = self.parse_error_pos {
                if line == err_line {
                    ...
                    draw_squiggle(
                        ui.painter(),
                        egui::pos2(sq_x, row_top + ROW_H - 2.0),
                        sq_w,
                        shell::ERR,
                    );
                }
    ...
    pub(crate) fn draw_text_side(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            if self.cursor_stack.is_empty() && self.cursor_stack_line == 0 {
                ...
                section_header(
                    ui,
                    &format!("stack at line {}", self.cursor_stack_line),
                    None,
                );
        ...
        ui.horizontal(|ui| {
            ...
            if !repl_candidates.is_empty() {
                ui.input_mut(|i| {
                    if i.consume_key(egui::Modifiers::NONE, egui::Key::ArrowDown) {
                        self.completion_idx =
                            (self.completion_idx + 1).min(repl_candidates.len().saturating_sub(1));
                    }
                    if i.consume_key(egui::Modifiers::NONE, egui::Key::ArrowUp) {
                        self.completion_idx = self.completion_idx.saturating_sub(1);
                    }
...
fn apply_completion_text(source: &mut String, line: usize, col: usize, word: &str) {
    ...
    let Some(line_str) = lines.get(line_idx) else {
        return;
    };
    let chars: Vec<char> = line_str.chars().collect();
    let col_idx = (col.saturating_sub(1)).min(chars.len());
    ...
    let line_byte_offset: usize = source.lines().take(line_idx).map(|l| l.len() + 1).sum();

...
use crate::{app::{ReplKind, StaxApp}, shell};
...
fn op_row(op: &Op) -> (&'static str, String) {
    match op {
        ...
        Op::BindMany { names, list_mode } => {
            let ns = names.iter().map(|s| s.as_ref()).collect::<Vec<_>>().join(" ");
        ...
        Op::MakeForm { keys, parent } => {
            let ks = keys.iter().map(|s| s.as_ref()).collect::<Vec<_>>().join(" ");
            ("form", if *parent { format!("^{{{ks}}}") } else { format!("{{{ks}}}") })
        }
        Op::MakeFun { params, body } => {
            let ps = params.iter().map(|s| s.as_ref()).collect::<Vec<_>>().join(" ");
            ("fun", format!("\\{ps}  [{} ops]", body.len()))
        }
        Op::Each { depth, order } => ("each",   format!("@depth={depth} ord={order}")),
        Op::Adverb(a)           => ("adverb",  match a {
            Adverb::Reduce    => "/",
            Adverb::Scan      => "\\",
            Adverb::Pairwise  => "^",
        }.into()),
...
fn fmt_val(v: &Value) -> String {
    match v {
        Value::Real(x) => {
            if *x == x.floor() && x.abs() < 1_000_000.0 { format!("{}", *x as i64) }
            else { format!("{x:.4}") }
        }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                kv_table(ui, "dbg_app", &[
                    ...
                    ("canvas_pan",    format!("{:.1},  {:.1}", self.canvas_pan.x, self.canvas_pan.y)),
                    ...
                    ("positions",     format!("{} nodes placed", self.node_positions.len())),
                    ("rank_ovr",      format!("{} entries", self.rank_overrides.len())),
                    ("adverb_ovr",    format!("{} entries", self.adverb_overrides.len())),
                ]);
                                    ...
                                    egui::Grid::new("dbg_graph_grid")
                                        ...
                                        .show(ui, |ui| {
                                            ...
                                            for (id, kind, label, ins, outs, adv) in &nodes {
                                                ...
                                                mono(ui, &format!("{ins}"), shell::PORT_STREAM, 10.0);
                                                mono(ui, &format!("{outs}"), shell::PORT_SIGNAL, 10.0);
                ...
                // ── STACK ────────────────────────────────────────────────
                let stack: Vec<_> = self.interp.stack.iter().rev()
                    ...
                    egui::Grid::new("dbg_tt_grid")
                        ...
                        .show(ui, |ui| {
                            for (i, snap) in self.travel_snapshots[start..].iter().enumerate() {
                                ...
                                mono(ui, &snap.label, if is_cur { shell::WARM } else { shell::INK_2 }, 11.0);
                                ui.end_row();
                            }
                        });
                }
...
fn row(ui: &mut egui::Ui, key: &str, val: &str, col: Color32) {
    ui.horizontal(|ui| {
        ...
        ui.add_sized(
            vec2(80.0, 16.0),
            egui::Label::new(egui::RichText::new(key).color(shell::INK_2).size(11.0).monospace()),
        );
...
fn mono_strong(ui: &mut egui::Ui, text: &str, col: Color32, size: f32) {
    ui.label(egui::RichText::new(text).color(col).size(size).monospace().strong());
}

fn empty(ui: &mut egui::Ui, msg: &str) {
    ui.horizontal(|ui| {
        ui.add_space(14.0);
        ui.label(egui::RichText::new(msg).color(shell::INK_3).size(11.0).monospace());
    });
}

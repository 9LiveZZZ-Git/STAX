...
impl StaxApp {
        ...
        if let Some(s) = cc.storage {
                ...
                for entry in rank_str.split(',').filter(|s| !s.is_empty()) {
                    ...
                    if parts.len() == 3 {
                        if let (Ok(nid_u), Ok(port_u), Ok(rank_u)) = (
                            ...
                            app.rank_overrides
                                .insert((stax_graph::NodeId(nid_u), port_u), rank_u);
                        }
                    }
                }
            }
            if let Some(adv_str) = s.get_string("adv_ovr") {
                for entry in adv_str.split(',').filter(|s| !s.is_empty()) {
                    let parts: Vec<&str> = entry.split(':').collect();
                    if parts.len() == 2 {
                        if let (Ok(nid_u), Ok(adv_u)) =
                            (parts[0].parse::<u32>(), parts[1].parse::<u8>())
                        {
                            let adv = match adv_u {
    ...
    pub fn compute_cursor_stack(&mut self) {
        if self.cursor_line == self.cursor_stack_line {
            return;
        }
        ...
        for line in &lines {
            ...
            let top = stax_parser::parse(&partial).ok().and_then(|ops| {
                ...
                interp
                    .exec(&ops)
                    .ok()
                    .and_then(|_| interp.stack.last().cloned())
            });
            results.push(top);
        }
    ...
    pub fn exec_repl(&mut self, line: &str) {
        self.repl_history.push(ReplLine {
            kind: ReplKind::Input,
            text: line.to_owned(),
        });
...
impl StaxApp {
    ...
    pub fn render_frame(&mut self, ctx: &egui::Context) {
        ...
        if self.show_dot_window {
            crate::dot::draw_dot_window(
                ctx,
                &mut self.show_dot_window,
                &mut self.dot_source,
                &self.graph,
            );
        }
        ...
        match self.view {
            ...
            View::Text => {
                egui::SidePanel::left("rf_files")
                    .exact_width(shell::LIB_W)
                    .frame(frame_none)
                    .show_separator_line(false)
                    .show(ctx, |ui| self.draw_files_panel(ui));
                egui::SidePanel::right("rf_side")
...
impl StaxApp {
        ...
        ui.horizontal(|ui| {
            ...
            egui::menu::menu_button(
                ...
                |ui| {
                    ...
                    if ui.button("fn-port").clicked() {
                        self.view = View::FnPort;
                        ui.close_menu();
                    }
            ...
            egui::menu::menu_button(
                ...
                |ui| {
                    ui.label(
                        egui::RichText::new("stax — sound as pure form")
                            .color(shell::INK_2)
                            .size(11.0)
                            .monospace(),
                    );
                    ui.separator();
                    ui.label(
                        egui::RichText::new("REPL commands:")
                            .color(shell::INK_3)
                            .size(10.0)
                            .monospace(),
                    );
                    ui.label(
                        egui::RichText::new(".s  show stack")
                            .color(shell::INK_2)
                            .size(10.0)
                            .monospace(),
                    );
                    ui.label(
                        egui::RichText::new(".c  clear stack")
                            .color(shell::INK_2)
                            .size(10.0)
                            .monospace(),
                    );
    ...
    fn draw_timebar(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if r.clicked() && total > 0 {
                self.travel_step = total.saturating_sub(1);
            }
            ui.add_space(4.0);
            ...
            let (bar_resp, bar_painter) =
                ui.allocate_painter(vec2(bar_w, shell::TIMEBAR_H - 8.0), egui::Sense::drag());
            ...
            if total > 1 {
                ...
                if bar_resp.dragged() {
                    if let Some(pos) = bar_resp.interact_pointer_pos() {
                        ...
                        self.travel_step =
                            ((t * (total - 1) as f32).round() as usize).min(total - 1);
                    }
                }
            }
        });
    ...
    pub(crate) fn draw_library(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            lib_group(
                ...
                "streams",
                &[
                    "take", "drop", "cycle", "zip", "by", "fold", "scan", "size", "reverse",
                ],
                &mut started,
            );
            lib_group(
                ...
                "signals",
                &[
                    "sinosc", "saw", "pulse", "wnoise", "pink", "combn", "pluck", "lpf", "hpf",
                    "ar", "adsr",
                ],
                &mut started,
            );
            ...
            lib_group(
                ui,
                "analysis",
                &["goertzel", "cqt", "mdct", "lpcanalz", "fft", "normalize"],
                &mut started,
            );
    ...
    pub(crate) fn draw_graph_canvas(&mut self, ui: &mut egui::Ui) {
        ...
        if response.drag_started() && !alt {
            if let Some(p) = ptr {
                if let Some((nid, port_idx)) = hovered_output {
                    ...
                    self.in_progress_wire = Some(stax_graph::PortRef {
                        node: nid,
                        port: port_idx,
                    });
            ...
            if let Some(src_ref) = self.in_progress_wire.take() {
                if let Some(drop_p) = self.wire_ghost_end.take() {
                    let dst = self.graph.nodes_in_order().find_map(|n| {
                        let sp = self.node_positions.get(&n.id).map(|&p| to_screen(p))?;
                        crate::graph::port_at_screen(drop_p, n, sp, zoom, false).map(|pi| {
                            stax_graph::PortRef {
                                node: n.id,
                                port: pi,
                            }
                ...
                if let Some(nid) = hit_node {
                    ...
                    'edge_search: for edge in self.graph.edges() {
                        let sp = self.graph.node(edge.src.node).and_then(|n| {
                            ...
                            Some(crate::graph::port_screen_pos(
                                to_screen(pos),
                                n,
                                edge.src.port,
                                true,
            ...
            if let Some(edge) = self.graph.edges().iter().find(|e| e.id == eid) {
                ...
                if let (Some(sp), Some(dp)) = (sp, dp) {
                    ...
                    painter.add(egui::Shape::line(
                        pts,
                        Stroke::new(2.0 * zoom.sqrt(), shell::WARM),
                    ));
                }
            }
        }
        ...
        let mut interact_zones: Vec<(stax_graph::NodeId, Vec<crate::graph::NodeInteract>)> =
            Vec::new();
        ...
        let ctx_hovered_edge: Option<stax_graph::EdgeId> = if ctx_hovered_node.is_none() {
            hover_pos.and_then(|hp| {
                self.graph.edges().iter().find_map(|edge| {
                    ...
                    match (sp, dp) {
                        (Some(s), Some(d)) if crate::graph::bezier_hit_test(s, d, hp, zoom) => {
                            Some(edge.id)
                        }
                        _ => None,
                    }
        ...
        response.context_menu(|ui| {
            if let Some(nid) = ctx_hovered_node {
                        ...
                        .clicked()
                {
                    ...
                    for did in &to_del {
                        ...
                        if self.selected_node == Some(*did) {
                            self.selected_node = None;
                        }
                    }
                    self.commit_graph_edit();
                ...
                if let Some((port_nid, port_idx, kind)) = ctx_hovered_input {
                    if port_nid == nid {
                        ui.separator();
                        if kind == stax_graph::PortKind::Fun && ui.button("View fn body").clicked()
                        {
                            ui.close_menu();
                        ...
                        if ui
                            .button(format!("Disconnect port in:{port_idx}"))
                            .clicked()
                        {
                            ui.close_menu();
                            let eids: Vec<_> = self
            ...
            if ctx_hovered_node.is_none() {
                if let Some(eid) = ctx_hovered_edge {
                    if ui.button("Delete wire").clicked() {
                        ui.close_menu();
                        self.graph.remove_edge(eid);
                        if self.selected_edge == Some(eid) {
                            self.selected_edge = None;
                        }
                        self.commit_graph_edit();
                    }
            ...
            ui.menu_button("Add node …", |ui| {
                let groups: &[(&str, &[&str])] = &[
                    ...
                    (
                        "streams",
                        &["ord", "nat", "by", "cyc", "N", "take", "drop", "zip"],
                    ),
                    (
                        "signals",
                        &[
                    ...
                    if rect.contains(ghost_screen) {
                        ...
                        let fun_port_target: Option<(NodeId, u8)> =
                            self.graph.nodes_in_order().find_map(|n| {
                                let sp = self.node_positions.get(&n.id).map(|&p| to_screen(p))?;
                                let pi =
                                    crate::graph::port_at_screen(ghost_screen, n, sp, zoom, false)?;
                        ...
                        let world_pos =
                            crate::graph::screen_to_canvas(ghost_screen, pan, zoom, origin);
                        if let Some((dst_nid, dst_port)) = fun_port_target {
                            ...
                            let src_ref = stax_graph::PortRef {
                                node: quote_nid,
                                port: 0,
                            };
                            let dst_ref = stax_graph::PortRef {
                                node: dst_nid,
                                port: dst_port,
                            };

...
| `stax-editor` | egui node editor — graph + text views (M5 ✅) |
...
**M5 (egui editor — graph + text views)** ✅ — `stax-editor`: native eframe/egui app with full app shell (40px header, 28px tabs, inspector right column, 28px status bar) matching design tokens exactly. **Graph view**: dot-grid canvas (24px spacing), cubic-bezier wires colored by PortKind (stream wires dashed, fun wires 2px), node rectangles with port squares, selection highlight (warm border + glow), drag-to-reposition, Alt+drag pan, scroll zoom, floating canvas header tab strip, bottom-left port-type legend. Auto-layout assigns column positions from topological depth. Rank badges (5-state cycle: none/`@`/`@1`/`@2`/`@@`) and adverb toggles (none/`/`/`\`/`^`) per-node, clickable with warm highlight. Scope strip on sink nodes. **Text view**: syntax-highlighted editor with line numbers (character-level lexer, 13px JetBrains Mono), breadcrumb bar, files/outline left panel, stack display + inspector + embedded REPL right panel. Cursor-line stack recomputation, did-you-mean suggestions. **REPL**: `.s` / `.c` commands, parse→exec via stax-eval, shows stack top. **Time-travel**: ring buffer of `TravelSnapshot`s after each exec, interactive scrub bar with clickable ◀◀ ◀ ❚❚ ▶ ▶▶ controls. **Persistence**: eframe storage saves/restores canvas_pan, canvas_zoom, and active view. **Reveal router**: `pending_reveal` queued cross-view jumps (GraphNode → pan to node, TextLine → switch to text + set cursor). **Fn-port view** (third tab): wires a selected fn node's signature/port detail panel. All 489 tests pass (no regressions).
